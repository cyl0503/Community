...
public class LoginController implements CommunityConstant {
  ...
  public String register(Model model,User user) {
    ...
    if (map == null || map.isEmpty()) {
      //注册成功后跳转操作结果页面页面
      model.addAttribute("msg","注册成功，我们已经成功向您发送一封邮件，请尽快激活！");
      //首页
  ...
  public String login(String username, String password, String code, boolean rememberme,
    ...
    //首次登录 cookie中没有ticket 跳过拦截器preHandle 直接进入controller中login()方法 重新生成的ticket
    //返回给客户端一个含有ticket的cookie 没有返回user对象 此时页面顶部并没有渲染到数据
    //所以重定向redirect:/index 浏览器和服务器之间的请求被重新刷新 这是一个新的请求 拦截器会重新拦截
